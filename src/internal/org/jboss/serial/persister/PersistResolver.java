...
public class PersistResolver {
	...
	static {
		try {
			...
			if (enumClass != null) {
				try {
					enumPersister = (Persister) PersistResolver.class.getClassLoader()
					    .loadClass("internal.org.jboss.serial.persister.EnumerationPersister").newInstance();
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public static Persister resolvePersister(Object objToBeSerialized, ClassMetaData metaData) {
		if (metaData.isArray() && (!(objToBeSerialized instanceof Class))) {
			return arrayPersister;
		} else if (objToBeSerialized instanceof Class || metaData.getClazz() == java.lang.Class.class) {
			return classReferencePersister;
