...
public class StreamingClass {
	...
	public static StreamingClass readStream(ObjectInput inp, ClassResolver resolver, ClassLoader loader)
	    throws IOException {
		...
		for (int slotIndex = 0; slotIndex < slots.length; slotIndex++) {
			...
			for (short fieldIndex = 0; fieldIndex < fields.length; fieldIndex++) {
				...
				if (fieldOnHash == null) {
					throw new IOException("Field hash " + fieldOnHash + " is not available on current classPath for class "
					    + className);
				}
